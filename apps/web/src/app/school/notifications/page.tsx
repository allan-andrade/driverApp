...
export default function SchoolNotificationsPage() {
  ...
  const { data = [], isLoading } = useQuery({
    queryKey: ['school-notifications-me'],
  ...
  const readAll = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['school-notifications-me'] }),
  });

  return (
    <main className="shell py-10">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-bold">Notificacoes da autoescola</h1>
        <button
